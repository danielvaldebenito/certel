...
public class CreatePDF
{
    ...
    int subpoint = 1;
    int page = 1;
    public static int NormaPrincipal = 6;
    public static string NormaPrincipalNombre = "NCh3395/1:2016";
    List<BookMark> bookMarkList = new List<BookMark>();
    public string Rendered { get; set; }
    public CreatePDF(Inspeccion inspeccion)
    {
    ...
    public void DefineCover(Document document)
    {
        ...
        paragraph = section.AddParagraph(string.Format("INFORME FASE {0} {1} N° {2}", Inspeccion.Fase, Inspeccion.Aparato.Nombre, Inspeccion.Numero));
        paragraph.Format.Font.Size = 12;
        paragraph.Format.Font.Bold = false;
        paragraph.Format.SpaceAfter = "1cm";
        paragraph.Format.Alignment = ParagraphAlignment.Center;
    ...
    public void BreveIntroAndAlcance()
    {
        ...
        Paragraph parrafo = section.AddParagraph(string.Format("El presente informe se refiere a los resultados de la Inspección de la Auditoría Técnica denominada Fase {0}, realizada a la instalación del {1}, ubicado en {2}.", Inspeccion.Fase, Inspeccion.Aparato.Nombre, Inspeccion.Ubicacion));
        ...
        subpoint++;
        parrafo1.Style = "Parrafo";
        parrafo1 = section.AddParagraph(string.Format("{0} \tEsta norma especifica los requisitos de seguridad que deben cumplir los ascensores y montacargas eléctricos emplazados en edificaciones cuyo permiso de edificación haya sido otorgado con anterioridad al 24 de octubre de 2010.", point + "." + subpoint + "."));
        subpoint++;
        parrafo1.Style = "Parrafo";
    ...
    public void Referencias()
    {
        ...
        Paragraph texto = section.AddParagraph(string.Format("{0}.{1}	NCh3395/1:2016 Equipos de transporte vertical - Parte 1: Requisitos para la inspección de ascensores y montacargas eléctricos existentes.", point, subpoint));
        texto.Style = "Parrafo";
        subpoint++;
        texto = section.AddParagraph(string.Format("{0}.{1}	Lista de Verificación Técnica NCh3395/1:2016.", point, subpoint));
        texto.Style = "Parrafo";
        subpoint++;
    ...
    public void ImagenCabina()
    {
        ...
        string pathImage = HttpContext.Current.Server.MapPath("~/css/images/");
        Image image = section.LastParagraph.AddImage(pathImage + "/cabina.png");
    ...
    public void TerminosYDefiniciones()
    {
        ...
        using (var db = new CertelEntities())
        {
            var terminos = db.TerminosYDefiniciones
                            .Where(w => w.NormaID == NormaPrincipal) // Nch:3395
                            .ToList();
    ...
    public void ResultadosInspeccion()
    {
        ...
        using (var db = new CertelEntities())
        {
            ...
            var n = Inspeccion
                            ...
                            .Where(w => w.Norma.TipoInformeID == 1)
                            .Select(s => s.Norma)
                            .FirstOrDefault();
            if (n == null)
                return; 
            ...
            foreach (var t in titulos)
            {
                ...
                foreach (var r in requisitos)
                {
                    ...
                    foreach (var c in cars)
                    {
                        ...
                        cRow.HeightRule = RowHeightRule.AtLeast;

                        var cumplimiento = c.Cumplimiento.Where(w => w.InspeccionID == Inspeccion.ID).FirstOrDefault();
    ...
    public void ObservacionesNormativasYTecnicas()
    {
        ...
        Paragraph texto = section.AddParagraph(string.Format("El edificio donde está emplazado el {0},  fue construido con anterioridad al  24 de octubre de 2010, y constituye la condición de ser un {0} existente. Por tanto, el {0} es inspeccionado bajo la Norma NCh3395/1:2106, según la Ley 20.296.", Inspeccion.Aparato.Nombre));
        texto.Style = "Parrafo";
    ...
    public void Conclusiones()
    {
        ...
        switch (tipoCalificacion)
        {
            case 0: // NO CALIFICA
                ...
                texto.Style = "Parrafo";
                break;
            case 1: // CALIFICA CON OBSERVACIONES MENORES
                texto = section.AddParagraph(string.Format("Es necesario dar solución a las no conformidades y observaciones encontradas, separando las correspondientes a la edificación (cliente), así como las correspondientes a la empresa mantenedora de ascensores,  con el objeto de incrementar la seguridad del mismo, proteger adecuadamente a los usuarios, a los técnicos de mantención y/o personal propio de la empresa en labores de rescate de emergencia."));
                texto.Style = "Parrafo";
                texto = section.AddParagraph(string.Format("La OGUC (Ordenanza General de Urbanismo y Construcciones) en el Artículo 5.1.6, Numeral 13, indica que los elevadores deben disponer de una carpeta cero (o carpeta del elevador), este requisito es reafirmado por el punto Registros, de la norma {0} que indica la documentación necesaria que debe disponer dicha carpeta.", NormaPrincipalNombre));
                texto.Style = "Parrafo";
                texto = section.AddParagraph(string.Format("Es importante que tanto la administración del Edificio {0}, como la empresa mantenedora, colaboren en la implementación de la carpeta cero,  ya que existen en ella documentos que servirán para inscribir el ascensor en la DOM (Dirección de Obras Municipales) según la indicación de la OGUC Artículo 5.9.5. Numeral 1, mediante una identificación con número único de registro del elevador.", Inspeccion.NombreEdificio));
                texto.Style = "Parrafo";
                texto = section.AddParagraph(string.Format("El {0} N° {1}), en su estado actual, CALIFICA PARA LA INSPECCIÓN CON OBSERVACIONES MENORES, según  las disposiciones contenidas en la Ley 20.296 y el D.S. N° 47 “Ordenanza General de Urbanismo y Construcciones” OGUC, modificado por el D.S. N° 37 – D.O. 22.03.2016 y en cumplimiento del Artículo 5.9.5 numeral 4: Certificación de ascensores, montacargas y escaleras o rampas mecánicas. Se recomienda  corregir las no conformidades y observaciones técnicas según la norma {2} señaladas en los puntos 4 y 5 del presente informe para que el {0} pueda cumplir con las normas Chilenas y pueda certificarse sin observaciones.", Inspeccion.Aparato.Nombre, Inspeccion.Numero, NormaPrincipalNombre));
                texto.Style = "Parrafo";
                texto = section.AddParagraph(string.Format("Se da un plazo de {0} días corridos a partir de la fecha del envío de este informe para realizar trabajos correspondientes a las mejoras y/o levantamiento de no conformidades del {1}.", Inspeccion.DiasPlazo.ToString() ?? "", Inspeccion.Aparato.Nombre));
                texto.Style = "Parrafo";
                ...
                texto = section.AddParagraph(string.Format("Si pasados los {0} días, no se han realizado las mejoras; entonces se deberá comenzar nuevamente con el proceso de certificación; materia de otra cotización.", Inspeccion.DiasPlazo.ToString() ?? ""));
                texto.Style = "Parrafo";
                break;
            case 2: // CALIFICA SIN OBSERVACIONES
    ...
    public string Rendering()
    {
        ...
        string filename = string.Format("Informe IT {0}_{1}.pdf", Inspeccion.IT.Replace("/", "-"), date);
